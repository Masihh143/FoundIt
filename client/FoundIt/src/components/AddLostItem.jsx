...
const AddLostItem = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const response = await lostAPI.create(submitData);
      
      if (response.status === 201) {
        navigate('/dashboard');
      }
    } catch (err) {
      setError(err.response?.data?.message || 'Failed to create lost item');
    } finally {
      setLoading(false);
    }
  };
    ...
    <div className="min-h-screen bg-gray-50">
      <div className="max-w-2xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        ...
        <div className="mb-8">
          ...
          <p className="text-gray-600">Help others find your lost item by providing details</p>
        </div>
